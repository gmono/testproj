...
Node *zip_get(byte *data,int index)
{
    ...
    for(;;)
    {
        ...
        if(size>index)
        {
            ...
            else return *(Node **)(data+ptr+index*sizeof(Node *));
        }
        else
        {
            //范围外 跳过范围
...
void zip_zip(byte **dptr)
{
    ...
    byte *temp=(byte *)malloc(sizeof(Node *)*512);//压缩数据缓冲区
    int nowp=0;//压缩数据长度记录 同时也是当前写入指针
    //处理开始块
    temp[nowp++]=data[0]==NULL? 0:1;
    for(int i=0;i<256;++i)
    {
        Node *now=((Node **)data)[i];//得到当前数据
        if(now)
        {
            ...
            if(isnull)
            {
                ...
                lsum=1;
                ((Node **)(temp+nowp+1))[lsum-1]=now;
                isnull=false;//转换为记录实体
            }
            else
            {
                //正在记录实体
                ((Node **)(temp+nowp+1))[++lsum-1]=now;//从记录块开始便宜lsum个位置记录具体数据
            }
        }
        else
        {
    ...
    temp[nowp++]=0;//记录结束标记
    byte *ndata=(byte *)malloc(sizeof(Node *)*nowp);
    memcpy(ndata,temp,nowp);
    free(*dptr);
    *dptr=ndata;
    free(temp);
}
...
//    ret->nexts=(byte *)malloc(sizeof(Node *)*256);//预分配模式
//    memset(ret->nexts,NULL,sizeof(Node *)*256);
    return ret;
}
///
/// \brief Node_Insert
...
void Node_Zip(Node *head)
{
    ...
    Item *items=(Item *)malloc(sizeof(Item)*8192);//支持8192级树
    int now=0;//当前栈指针
    Item ti={head,0};//压入头
    items[0]=ti;
    while(now>=0)
    {
        ...
        {
            Node *child=NULL;
            while(items[now].cptr<256)
            {
                child=((Node **)nowitem.ptr->nexts)[items[now].cptr++];//得到当前子节点
                //nowitem.cptr++是错误的
                if(child)
                {
                    //取到一个可用的子节点
...
void Node_ZInsert(Node *head,const char *key,char *value,bool isreset)
{
    //尚未实现
}
